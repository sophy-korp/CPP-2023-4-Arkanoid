...
bool win() {
	//проверяем что остались только неразрушимые блоки
	bool f = true;
...
void game::mechanic() {
	if (ball::balls.empty() || win()) {
		Sleep(1000); //задержка чтоб увидеть, почему проиграли/выиграли
		exit(0);
	}
	game_object::apdateMainTime();
	//двигаем всё что двигается и проверяем на жизнеспособность
